...
rowCallback <- c(
  ...
  hints = {
    "Зарплата": "",
    "Full-time": "Пятидневка с 9 до 18. Работать необходимо оффлайн в офисе",
    "Гибрид": "2-3 дня удаленной работы, остальные дни рабочей недели необходимо лично присутствовать в офисе",
    "Формальная":"Строгая иерархия, высокая субординация,  общение на “вы” внутри коллектива,  мониторинг и контроль отработанного времени сотрудников, строгий  дресс-код. Руководство по принципу: я сказал - вы сделали",
    "Полуформальная":"Есть некоторая субординация и границы, связанные со статусом сотрудников, руководство готово вовлекать сотрудников в обсуждение рабочих моментов, классический дресс-код без строгих ограничений",
    "Неформальная":"Субординация не важна, развитые горизонтальные связи, индивидуальный подход к каждому сотруднику, общение на “ты” внутри коллектива, минимальный мониторинг сотрудников или его полное отсутствие, требования о дресс-коде отсутствуют",
    "Премиальный":"Офис высокого класса с панорамными видами, обилием естественного света, эргономичной мебелью премиум-класса и креативными зонами (капсулами для работы, комнатами релаксации и проч.)",
    "Современный":"Современный офис с open plan и хорошим освещением, просторный и оборудованный удобной мебелью. Есть места для совместной работы, coffee points",
    "Традиционный":"Базовый офис с функциональной, но устаревшей мебелью, классической планировкой и минимальным набором удобств",
...
function(input, output, session) {
  ...
  output$button_ui <- renderUI({
    if (current_page() == 1) {
      ...
      actionButton("button_2", "Далее")
        ...
        actionButton("button_3", "Далее")
    } else if (current_page() == 4) {
      actionButton("button_4", "Начать опрос")
    } else {
      actionButton("OK", "OK")
    }
  })
  ...
  observeEvent(input$button_3, {
    current_page(4)
    output$intro <- renderUI({
      HTML(readChar('intro_4.txt', file.info('intro_4.txt')$size))
    })
  })
